...
(function () {
  ...
  function renderStepReview(host) {
    ...
    host.innerHTML = ''
      ...
      + summary('Property', '<strong>' + esc(p.title) + '</strong><br>' + p.type + ' · ' + esc(destLabel(p.destination_id)) + ' · ' + (p.bedrooms === 0 ? 'Studio' : p.bedrooms + 'BR') + ' / ' + p.baths + ' bath · sleeps ' + p.max_guests, 2)
