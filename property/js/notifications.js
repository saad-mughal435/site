...
(function () {
  ...
  // ---------- Toast stack ----------
  function getStack() {
    var el = document.querySelector('.m-toast-stack');
    if (!el) {
      el = document.createElement('div');
      el.className = 'm-toast-stack';
  ...
  function loadNotifications() {
    try {
      return JSON.parse(localStorage.getItem('manzil.notifications') || '[]');
    } catch (e) { return []; }
  }
  ...
  function renderBell(host) {
    ...
    host.innerHTML = ''
      + '<button class="m-pill m-bell" type="button" aria-label="Notifications" data-bell-toggle>'
      +   '🔔'
      +   (unread ? '<span class="m-bell-dot"></span>' : '')
      + '</button>'
      + '<div class="m-bell-panel" data-bell-panel>'
      ...
      +     '<button class="m-pill m-pill--ghost m-btn--sm" data-bell-mark>Mark all read</button>'
      +   '</div>'
      +   list.map(function (n) {
            return '<div class="m-bell-item' + (n.unread ? ' unread' : '') + '">'
              +    '<div>' + escapeHtml(n.title) + '</div>'
              +    '<div class="when">' + escapeHtml(n.body) + ' · ' + relTime(n.when) + '</div>'
              +    '</div>';
          }).join('')
      +   (list.length === 0 ? '<div class="m-empty">No notifications yet.</div>' : '')
      + '</div>';
    ...
    var mark = host.querySelector('[data-bell-mark]');
