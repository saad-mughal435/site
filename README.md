...
+ encrypted Anthropic key. Structured data for search engines, working
...
| [`/vacation/`](https://saadm.dev/vacation/) | **Vacation Homes** - UAE short-stay booking demo. 54 stays across 10 destinations (Marina, Palm, Hatta, RAK, Liwa, etc.), custom hand-rolled date-range picker + availability calendar with conflict check, per-night pricing breakdown (nightly + weekend surcharge + cleaning + service + 5% VAT). **Host-side: 6-step listing wizard** (save-and-resume drafts, map-pin selector, document upload - Emirates ID + ownership + DTCM permit + IBAN) feeding an **admin verification queue** with approve / request-changes / reject. 13-section admin SPA + host dashboard with listings / bookings / calendar / earnings / profile / verification. EN/AR + 4-currency switcher. |
| [`/`](https://saadm.dev/) (chat bubble) | **Ask Saad** - recruiter AI chatbot on the homepage. Floating ✦ cyan bubble bottom-right. Recruiters ask plain-English questions ("Does he know Python?" / "What did he build at Kingsley?" / "Can he relocate?") and get 2-4 sentence answers grounded in a 40-doc RAG corpus extracted from the existing homepage content (hero copy, FAQ, experience, projects, skills, JSON-LD). Each answer renders one-click citation chips that drill into the relevant demo / role / FAQ. Live + mock modes - same Cloudflare Worker + Anthropic key as Sanad/Watad. See [`ask/README.md`](ask/README.md). |
...
| [`/nabta/`](https://saadm.dev/nabta/) | **Nabta** - UAE HR + payroll SaaS. 32 employees across 5 departments with full Emirates ID + visa fields, leave management with line-manager + HR approval workflow, **WPS-compliant payroll runs** via Emirates NBD (per-employee breakdown + Generate WPS SIF flow), recruitment kanban (4 open roles, 22 candidates), performance review cycle (Q2-2026), and a Claude HR policy assistant grounded in 6 company policies + UAE Labour Law (Federal Decree-Law No. 33 of 2021) with `[pol-xxx]` citation chips. The sixth AI integration. See [`nabta/README.md`](nabta/README.md). |
